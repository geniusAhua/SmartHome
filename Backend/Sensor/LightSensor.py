...
class Sensor:
    ...
    async def __respond_to_interest_with_data(self, dataName, _fileName, params, fromName, from_ws):
        ...
        if fileName == ".debug":
            ...
            sensorData = {}
            sensorData["brightness"] = self.__brightness
    ...
    async def __generate_temperature_and_humidity(self):
        while True:
            self.__brightness = random.randint(0, 100)
            self.__echo(f"[{self.__nodeName}] generate brightness ===> brightness: {self.__brightness}")
