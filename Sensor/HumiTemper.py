...
class Sensor:
    ...
    async def __respond_to_interest_with_data(self, _packet, _fileName, fromName, from_ws):
        ...
        if fileName == ".debug":
            ...
            sensorData = {}
            sensorData["humidity"] = self.__humidity
    ...
    async def __generate_temperature_and_humidity(self):
        while True:
            self.__temperature = random.randint(10, 30)
            self.__humidity = random.randint(40, 70)
            self.__echo(f"[{self.__nodeName}] generate temperature and humidity ===> temperature: {self.__temperature}, humidity: {self.__humidity}")
            await asyncio.sleep(5)

    async def __main(self):
        self.task = asyncio.create_task(self.__generate_temperature_and_humidity())
